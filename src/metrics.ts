...
import { jaccardSimilarity, computeLengthStats, embeddingCosineSimilarity } from './similarity';
import { enrichOutput } from './diff';
...
export function estimateCost(
  ...
  if (output.cost !== undefined) return output.cost;

  if (!output.model || !output.tokens) return undefined;
...
export async function computeMetrics(
  ...
  const a = enrichOutput(outputA);
  const b = enrichOutput(outputB);

  // Token counts
  const tokens: DiffMetrics['tokens'] = {
  ...
  // Similarity
  const jaccard = jaccardSimilarity(a.text, b.text);
  ...
  if (options?.embedFn) {
    ...
    semantic = embeddingCosineSimilarity(embA, embB);
  }

  // Length
  const lengthA = computeLengthStats(a.text);
  const lengthB = computeLengthStats(b.text);
